...
__all__ = ['Quantizemodels',  'QuantizeConv2d', 'QuantizeLinear', 'QuantizeAct',
           'truncation', 'get_mask', 'FunPopGradient', 'round_fun', 'grad_scale']


class Quantizemodels(Enum):
    layer_wise = 1
    kernel_wise = 2
...
class FunPopGradient(torch.autograd.Function):
...
def get_default_kwargs_q(kwargs_q, layer_type):
    ...
    if isinstance(layer_type, QuantizeConv2d):
        default.update({
            'mode': Quantizemodels.layer_wise})
    elif isinstance(layer_type, QuantizeLinear):
        pass
    elif isinstance(layer_type, QuantizeAct):
        pass
        # default.update({
        #     'signed': 'Auto'})
    else:
...
class QuantizeConv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
        ...
        super(QuantizeConv2d, self).__init__(in_channels, out_channels, kernel_size, stride=stride,
        ...
        if self.q_mode == Quantizemodels.kernel_wise:
    ...
    def extra_repr(self):
        s_prefix = super(QuantizeConv2d, self).extra_repr()
...
class QuantizeLinear(nn.Linear):
    def __init__(self, in_features, out_features, bias=True, **kwargs_q):
        super(QuantizeLinear, self).__init__(in_features=in_features, out_features=out_features, bias=bias)
        self.kwargs_q = get_default_kwargs_q(kwargs_q, layer_type=self)
        self.nbits = kwargs_q['nbits']
        if self.nbits < 0:
            self.register_parameter('alpha', None)
    ...
    def extra_repr(self):
        s_prefix = super(QuantizeLinear, self).extra_repr()
        if self.alpha is None:
            return '{}, fake'.format(s_prefix)
        return '{}, {}'.format(s_prefix, self.kwargs_q)


class QuantizeAct(nn.Module):
    ...
    def extra_repr(self):
        # s_prefix = super(QuantizeAct, self).extra_repr()
        if self.alpha is None:
            return 'fake'
