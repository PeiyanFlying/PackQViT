...
class Q_Attention(nn.Module):
    ...
    def __init__(self, nbits, dim, num_heads=8, quantize_attn=True, qkv_bias=False, attn_drop=0., proj_drop=0.):
        ...
        # self.norm = nn.BatchNorm1d(dim * 3)
    ...
    def forward(self, x):
        ...
        q, k, v = qkv.unbind(0)   # make torchscript happy (cannot use tensor as tuple) batchsize, head_number, N, sub_C [172, 3, 198, 64]
        q = self.norm_q(q)
        k = self.norm_k(k)
...
def fourbits_deit_tiny_patch16_224(pretrained=False, **kwargs):
    ...
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = checkpoint = torch.load('./pretrained/deit_t_best_checkpoint_4bit.pth',map_location="cpu")
        model.load_state_dict(checkpoint["model"], strict=False)
        print('that ok....')
    return model

@register_model
def eightbits_deit_tiny_patch16_224(pretrained=False, **kwargs):
    model = lowbit_VisionTransformer(
        nbits=8, patch_size=16, embed_dim=192, depth=12, num_heads=3, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = checkpoint = torch.load('./pretrained/deit_t_best_checkpoint_4bit.pth',map_location="cpu")
